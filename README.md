...
| --- | --- | --- |
| `#/` | Introduction | See what the explorer covers and jump into any section. |
...
| `#/data` | Data & integration | How statement data reaches Sepire (client → Sepire), a payload explorer tied to the statement and PDF, REST and GraphQL sandboxes with validation, and an "edit & re-render" view. Downloads: sample payload, JSON Schema, OpenAPI 3.1 and the GraphQL schema. |

The Data section is a sandbox: requests are simulated in the browser and nothing leaves it.
...
- **`scripts/gen-schemas.mjs`** derives the integration contract from `statement.json`: GraphQL
  SDL, JSON Schema 2020-12, OpenAPI 3.1 and the sample payload. It leaves out the Sepire-side keys
...
- **Live data.** With `?live=1`, the statement reads its payload from the explorer
  (`window.__SEPIRE_STATEMENT__`). The Data section validates edits with the real GraphQL schema
  (graphql-js) plus content rules. Printed and machine values must agree (an error). A balance
  bridge that doesn't reconcile, or percentages that don't total 100, raise warnings.
...
| Statement data | Replace `source/statement.json`. The payload and schemas regenerate on the next dev or build run. |
...
src/components/       app shell: top bar, theme toggle, statement stage (device frames)
...
src/sections/data     Data & integration (payload, REST, GraphQL, live edit)
